...
set_tmux_env PICKER_COPY_COMMAND "pbcopy"
# set_tmux_env PICKER_COPY_COMMAND_UPPERCASE "bash -c 'arg=\$(cat -); tmux split-window -h -c \"#{pane_current_path}\" vim \"\$arg\"'"
set_tmux_env PICKER_COPY_COMMAND_UPPERCASE 'pbcopy && tmux set-buffer "$(pbpaste)" && tmux paste-buffer'
...
set_tmux_env PICKER_HINT_FORMAT $(process_format "#[fg=white,bg=red,bold]%s")
